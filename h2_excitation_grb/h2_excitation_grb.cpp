...
energy_loss_data_unit enloss_rate, enloss_rate_aux, enloss_int;
double enloss_rate_h2_rot_pos, enloss_rate_h2_vibr_pos, energy_in_rovibr_levels;
...
vector<double> enloss_rate_h2_rot_arr_pos, enloss_rate_h2_vibr_arr_pos;

// Energy loss in [eV cm-3] during the time interval [0, t] as a function of time t,
vector< energy_loss_data_unit > enloss_int_arr;
...
//
// Parameters used only in GRB-H2 simulations
...
{	
	...
	//
	//
	// Simulations with mono-energetic electron spectrum
	//
...
#pragma omp master 
	...
	while (!input.eof())
	{
		...
		// Nb of H column densities to calculate:
		do
			input.getline(text_line, MAX_TEXT_LINE_WIDTH);
		while (text_line[0] == '#');
		...
		hcolumn_density_arr.clear();
		for (i = 0; i < nb; i++) {
			ss >> x;
			hcolumn_density_arr.push_back(x);
		}
		input.close();
	}
...
#pragma omp master 
	...
	output_path = "C:/Users/Александр/Documents/Данные и графики/paper GRB Numerical model of fast electron energy deposition/python_scripts_ism/";
	data_path = "C:/Users/Александр/Documents/input_data/";

	ioniz_fract = 1.e-7;
	str = "1e-7";
	ji = 0;  // check initial J of H2 (0 or 1)
	//save_cross_section_table(output_path, data_path, ioniz_fract, THERMAL_EL_TEMPERATURE, ji, str);
...
void memory_freeing_up()
{
	...
	enloss_rate.set_to_zero();
	enloss_rate_aux.set_to_zero();
	enloss_int.set_to_zero();

	enloss_rate_h2_rot_pos = enloss_rate_h2_vibr_pos = 0.;
	...
	for (i = 0; i < NB_EXC_ELECTRONIC_STATES; i++) {
		h2_state_data[i].set_to_zero();
		h2_state_data_rates[i].set_to_zero();
		h2_state_data_aux[i].set_to_zero();
	}
	...
	double op_ratio_h2, double electron_conc, double electron_energy)
{
	...
	double x1, x2, rel_tol, dt, model_time, model_time_aux, model_time_aux_prev, model_time_step, model_time_out;
	...
	// Energy loss rates
	enloss_rate_arr.push_back(enloss_rate);
	enloss_rate_h2_rot_arr_pos.push_back(enloss_rate_h2_rot_pos);
	enloss_rate_h2_vibr_arr_pos.push_back(enloss_rate_h2_vibr_pos);

	// Energy losses, time-integrated - check that all variables at this step are set to zero,
	enloss_int_arr.push_back(enloss_int);
	...
	while (model_time < time_cloud_arr.back())
	{
		...
		if (flag == CV_SUCCESS) {
			...
			// Energy losses, 
			enloss_rate_aux = enloss_rate;

			user_data->get_el_energy_losses(enloss_rate, enloss_rate_h2_rot_pos, enloss_rate_h2_vibr_pos);

			enloss_int.rot += (enloss_rate_aux.rot + enloss_rate.rot) * dt;
			enloss_int.vibr += (enloss_rate_aux.vibr + enloss_rate.vibr) * dt;
			enloss_int.vibr_01 += (enloss_rate_aux.vibr_01 + enloss_rate.vibr_01) * dt;
			enloss_int.ioniz += (enloss_rate_aux.ioniz + enloss_rate.ioniz) * dt;
			enloss_int.hei += (enloss_rate_aux.hei + enloss_rate.hei) * dt;
		
			// contribute to thermal heating
			enloss_int.mt += (enloss_rate_aux.mt + enloss_rate.mt) * dt;
			enloss_int.coulomb += (enloss_rate_aux.coulomb + enloss_rate.coulomb) * dt;
			enloss_int.neutral_coll_heating += (enloss_rate_aux.neutral_coll_heating + enloss_rate.neutral_coll_heating) * dt;
	...
	double hcolumn_density, int is_h2_pop_dens_init)
{
	...
	double conc_h_tot, op_ratio_h2, grb_distance, grb_cloud_distance;
	double x1, x2, rel_tol, dt, model_time, model_time_aux, model_time_aux_prev, model_time_step, model_time_out;
	...
	// updating H column density for given layer number, from the cloud boundary to the cloud layer centre,
	...
	user_data 
		= new elspectra_evolution_data (data_path, output_path, conc_h_tot, IONIZATION_FRACTION_THERMAL_EL, electron_energies_grid, VERBOSITY);
	...
	for (i = 0; i < nb_of_el_energies; i++) {
		// number of electrons in the interval:
...
// initialization of parameters of the gas-dust clooud
void init_cloud_parameters(const string& sim_path, double& grb_cloud_distance, double & conc_h_tot, double& op_ratio_h2)
{
	int i;
	char text_line[MAX_TEXT_LINE_WIDTH];
	...
	fname = sim_path + "grb_parameters.txt";
	input.open(fname.c_str());
	...
	while (!input.eof())
	{
		...
		ss.clear();
		ss.str(text_line);
		ss >> grb_cloud_distance;  // in cm, distance from the GRB source to the cloud boundary,
...
//
void init_specimen_conc(const string& path, vector<double>& layer_centre_distances, vector<dynamic_array>& d)
{

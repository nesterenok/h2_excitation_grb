...
//
// Electronic excited states: S1g(X), S1u(B,Bp), P1u(C+-,D+-)  
// S1g(X) -> S1u(B,Bp), singlet transition, dLambda = 0, Lambda = S = 0 
// S1g(X) -> P1u(C+-,D+-), singlet transition, dLambda = 1, Lambda = P = 1 
//

struct el_spectra_data {
    int i0, nb;
    double* weights;
    el_spectra_data() : i0(0), nb(0), weights(0) { ; }
    el_spectra_data(const el_spectra_data& obj);
    el_spectra_data& operator = (const el_spectra_data& obj);
};
...
protected:
    ...
    int el_nb, h_nb, hp_nb, h2_nb, h2p_nb, he_nb, hep_nb, hepp_nb, 
        h2_ioniz_min_nb, he_ioniz_min_nb, h_ioniz_min_nb, h2p_ioniz_min_nb, hep_ioniz_min_nb, h2_b1su_min_nb, h2_c1pu_min_nb,
        h2_bp1su_min_nb, h2_d1pu_min_nb, h2_b1su_diss_min_nb, h2_c1pu_diss_min_nb, h2_bp1su_diss_min_nb, h2_d1pu_diss_min_nb, h2_b3su_diss_min_nb,
        hei_min_nb, nb_lev_h2, nb_lev_h2_b, nb_lev_h2_cplus, nb_lev_h2_cminus, nb_lev_h2_bp, nb_lev_h2_dplus, nb_lev_h2_dminus, nb_lev_hei,
        nb_coll_trans_hei, nb_of_el_energies, nb_of_equat, h2eq_nb, heieq_nb, physeq_nb, min_grain_charge;
    
    double conc_h_tot, grain_cs, grain_nb_density,
        enloss_deriv_mt, enloss_deriv_h2_rot, enloss_deriv_h2_vibr, enloss_deriv_h2_electr, enloss_deriv_h2_electr_tr, enloss_deriv_ioniz,
        enloss_deriv_coloumb_ions, enloss_deriv_coloumb_el, enloss_deriv_hei, conc_n, conc_i, energy_gain_n, energy_gain_i, nb_gain_n, nb_gain_i, 
    ...
    double *el_enloss_h2_mt, *el_enloss_he_mt, *h2_ioniz_rates_tot, *he_ioniz_rates_tot, *h_ioniz_rates_tot, *h2p_ioniz_rates_tot, *hep_ioniz_rates_tot, 
        *h2_j02_rates, *h2_j24_rates, *h2_j13_rates, *h2_j35_rates, * h2_j20_rates, * h2_j42_rates, * h2_j31_rates, * h2_j53_rates,
    ...
    double** h2_ioniz_rates, ** he_ioniz_rates, ** h_ioniz_rates, ** h2p_ioniz_rates, ** hep_ioniz_rates, 
        **h2_v0_bstate_rates, ** h2_v1_bstate_rates, ** h2_v0_cstate_rates, ** h2_v1_cstate_rates, ** h2_v0_bpstate_rates, ** h2_v1_bpstate_rates,
    ...
    void init_tables_ionization(electron_impact_ionization*, double *& rates_tot, double**& rates, spectra_data**& indexes, int &el_ion_min_nb, int verbosity = 1);
    void init_tables_coulomb(electron_impact_ionization*, double*&, double**&, el_spectra_data**&, int verbosity = 1);
    ...
    void init_tables_h2_electronic_exc(const energy_diagram * h2_di_elexc, cross_section_table_mccc**, double**& rates, spectra_data**& indexes,
    ...
    void derivatives_ionization(const realtype* y_data, realtype* ydot_data, 
        const electron_impact_ionization*, double** rates, const double* rates_tot, spectra_data** indexes,
    ...
    void derivatives_h2_rotational_exc(const realtype* y_data, realtype* ydot_data, const double* rates_up, const spectra_data* indexes_up, 
        const double* rates_down, const spectra_data* indexes_down, int j0, double & enloss_deriv);
    ...
    // all parameters with & (energy loss and dissociation rate) are summed,
    void derivatives_hei_exc(const realtype* y_data, realtype* ydot_data, double& enloss_deriv, double& exc_rate);
...
public:
    ...
    // energy loss via excitation of singlet (h2_electr) and triplet states (h2_electr_tr), including excitation and dissociative excitation for singlet states,
    void get_el_energy_losses(double & mt, double & h2_rot, double &h2_vibr, double & h2_electr, double & h2_electr_tr, double & ion, 
